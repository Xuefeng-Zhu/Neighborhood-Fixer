...
The AWS application is deployed in **us-west-2** at [Neighborhood Fixer](https://main.d12il66dljooo6.amplifyapp.com). The current source uses Clerk sign-in and public signup with one server-assigned Demo Borough, daily quotas and generation-fenced admission. Live Clerk validation and a reviewed clean reset must finish before public access opens. Historical AWS checks verified maps, private storage, two residents' real Bedrock photo analyses, routing and one immutable shared draft using the previous login integration. Those checks stopped before approval. **Full AWS behavior remains unverified:** AgentCore Browser streams return HTTP 404 before navigation, blocking submission and receipt verification. See [current AWS status](docs/AWS-STATUS.md) for the evidence boundary.
