...
The AWS application is deployed in **us-west-2** at [Neighborhood Fixer](https://main.d12il66dljooo6.amplifyapp.com) with Clerk public signup, one server-assigned Demo Borough, daily quotas and generation-fenced admission. Hosted signup, JWT binding, renewal/logout, the Amazon Location map, private storage, report quotas, and two residents' real Bedrock/Strands photo-to-shared-draft journey passed. The reviewed reset left one inert sample, no QA identities or uploads, and public admission; CloudFormation contains zero Cognito resources and the former pool was deleted. The cloud journey stopped before approval. **Full AWS behavior remains unverified:** AgentCore Browser streams return HTTP 404 before navigation, blocking submission and receipt verification. See [current AWS status](docs/AWS-STATUS.md) for the evidence boundary.
