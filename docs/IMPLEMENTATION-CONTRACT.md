...
## Official contact research and internal outreach

Official-contact research is an owner-only workflow independent of fictional portal submission. The API reverse-geocodes the incident's stored coordinates and returns only an administrative Seattle candidate bound to the current case context. The resident confirms that candidate before the worker constructs a fixed `Seattle WA + category` Brave query. The worker validates official HTTPS URLs, performs bounded HTML reads, extracts only role-based public contacts, and exposes at most three normalized candidates from a 24-hour encrypted transient cache. Selecting one copies only that contact/source snapshot to retained case storage and deletes the unselected set. The client cannot supply jurisdiction or query text.

Selecting a candidate is separate from approving an action. Contact refreshes, selection changes, or material case changes stale channel drafts. Email simulation approval binds the exact subject, body, selected-contact snapshot, internal sink, context, revision, and payload hash. Execution writes one idempotent internal receipt with `SIMULATED_NOT_SENT`; it has no transport adapter.

Voice simulation approval binds the selected-contact snapshot, minimized fact envelope, allowed intents, refusal rules, six-turn limit, 90-second limit, internal sink, and payload hash. Two fresh tool-free agent roles may propose bounded turns; deterministic validation rejects unsupported roles, intents, fact references, claims, lengths, or contact artifacts. Polly receives only validated caption text. In AWS, a dedicated audio Lambda verifies the Clerk JWT in-function, reuses the domain's owner and header-only playback-capability checks, and streams the Polly body through Lambda Web Adapter response streaming plus API Gateway REST `ResponseTransferMode.STREAM`. The main API cannot synthesize because it has no Polly permission. Full turns live in temporary encrypted storage, are actively purged at the advertised expiry or on completion/cancellation, and carry a 15-minute TTL fallback. Retained state contains a receipt, hash, and bounded summary with `SIMULATED_NOT_DIALED`.
...
Unsupported routing yields clarification or an assisted handoff packet. Real municipal submissions, email delivery, and dialing remain disabled. Integration health reports distinguish configured, tested, unavailable and simulated states; errors never switch AWS operations into fixtures.
